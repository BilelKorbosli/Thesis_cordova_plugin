 ...
 */
public class FreeStyleLibrePlugin extends CordovaPlugin implements NfcAdapter.OnNdefPushCompleteCallback {
    ...
    private static final String DUMPDATA = "dumpData";
    ...
    public boolean execute(String action, JSONArray data, CallbackContext callbackContext) throws JSONException {
        ...
        if (action.equalsIgnoreCase(DUMPDATA)) {
            dumpData(callbackContext);
        ...
        } else if (action.equalsIgnoreCase(GLUCOSE_CONVERT)) {
            glucoseConvert(data.getString(0),callbackContext);

        } else if (action.equalsIgnoreCase(DUMPDATA)) {
            dumpData(callbackContext);
    ...
    private void startNdefBeam(final CallbackContext callbackContext, final Uri[] uris) {
        getActivity().runOnUiThread(() -> {
            ...
            if (nfcAdapter == null) {
                ...
                nfcAdapter.setOnNdefPushCompleteCallback(FreeStyleLibrePlugin.this, getActivity());
                try {
    ...
    // Conver BufferByte to value
    private void glucoseConvert(String bufferHex, CallbackContext callbackContext ){
        final String g = bufferHex.substring(2,4)+bufferHex.substring(0,2);
        
          // ((0x4531 & 0xFFF) / 6) - 37;
          int bitmask = 0x0FFF;
          Float val = Float.valueOf( Float.valueOf((Integer.parseInt(g,16) & bitmask) / 6) - 37);
        ...
        cordova.getThreadPool().execute(() -> {
            try {
                ...
                for(int i=3; i <= 40; i++) { 
                    ...
                    allBlocks[i - 3] = Arrays.copyOf(response,1, response.length);
                    try{
